  ...
  return content?.toString().length ? (
    <>
      <div className="grid grid-cols-2 gap-1 content-center items-center dark:text-white">
        ...
        <div className="col-span-1 text-black dark:text-white">{`${content}`}</div>
      </div>
      <Separator />
    </>
  ) : null;
...
export function DescriptionItemWithMap({ title, content, item }: DescriptionItemWithMapProps) {
  ...
  if (title.includes("На карте (Точки)")) {
    linkTo = (
      <Link to={link_to_map_stp} target="_blank" rel="noopener noreferrer" className={buttonVariants({ variant: "link", size: 'sm' })}>
        Показать на карте
      </Link>
    );
    showBtn = !!item?.rgf;
    ...
    linkTo = (
      <Link to={link_to_map_stl} target="_blank" rel="noopener noreferrer" className={buttonVariants({ variant: "link", size: 'sm' })}>
        Показать на карте
      </Link>
    );
    showBtn = !!item?.rgf;
    ...
    linkTo = (
      <Link to={link_to_map_sta} target="_blank" rel="noopener noreferrer" className={buttonVariants({ variant: "link", size: 'sm' })}>
        Показать на карте
      </Link>
    );
    showBtn = !!item?.rgf;
...
export default function ReportDrawer({ open, onClose, item }: PropsDrawer) {
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      ...
      <SheetContent className="max-w-5xl sm:max-w-5xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Подробнее об отчете:</SheetTitle>
          <SheetDescription id="report-drawer-description">
            Данные об отчете, включая информацию о карте, авторах и других параметрах.
          </SheetDescription>
        </SheetHeader>
        <div className="py-4" aria-describedby="report-drawer-description">
          <Separator />
          ...
          <DescriptionItem
            ...
            content={item?.lastupdate ? new Date(item.lastupdate).toLocaleDateString('ru-RU') : ''}
          />
          {isLoading ? (
